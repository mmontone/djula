...
;; Fix to pathnames comparisons in ECL
;; See https://github.com/mmontone/djula/issues/90
(defun system-relative-pathname (system name &key type)
  #-ecl(asdf:system-relative-pathname system name :type type)
  #+ecl(let ((path (asdf:system-relative-pathname system name :type type)))
         (merge-pathnames path (make-pathname :version :newest))))

(def-test find-template-test (:compile-at :definition-time)
  (set-current-path nil)
  (is (equalp (djula:find-template* "t1.djula" nil)
              (system-relative-pathname :djula "test/templates/t1.djula")))
  (is (null (djula:find-template* "../t1.djula" nil)))
  (is (null (djula:find-template* "t14.djula" nil)))
  (is (equalp (djula:find-template* "subdir/t14.djula" nil)
              (system-relative-pathname :djula "test/templates/subdir/t14.djula")))
  (set-current-path (system-relative-pathname :djula "test/templates/subdir/"))
  (is (equalp (djula:find-template* "t1.djula" nil)
              (system-relative-pathname :djula "test/templates/t1.djula")))
  (is (equalp (djula:find-template* "./t1.djula" nil)
              (system-relative-pathname :djula "test/templates/subdir/t1.djula")))
  (is (equalp (djula:find-template* "../t1.djula" nil)
              (system-relative-pathname :djula "test/templates/t1.djula")))
